...
export function SidebarNav({ onNavigate, className }: SidebarNavProps) {
  const { user, signOut, loading, districtName } = useAuth()
  ...
  return (
    <div className={cn('flex h-full flex-col', className)}>
      <div className="border-b border-border px-4 py-5">
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">{districtName ?? 'Municipio'}</p>
      </div>
      ...
      <div className="border-t border-border p-3">
        ...
        <Button
          ...
          onClick={() => {
            void signOut().then(() => {
              window.location.assign('/select-municipio')
            })
          }}
        >
